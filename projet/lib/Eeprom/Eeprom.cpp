...
# warning "F_CPU pas defini pour 'Eeprom.cpp'"
# define F_CPU 8000000UL
#endif

uint8_t Eeprom::m_adresse_peripherique = 0xA0;
...
/******************************************************************************/
/* uint8_t Eeprom::choisir_banc(const uint8_t banc)                    */
/*                                                                            */
...
uint8_t Eeprom::choisir_banc(const uint8_t banc) {
   ...
   uint8_t rv = 255;
   if(banc == temp) {
      Eeprom::m_adresse_peripherique = (0xA0 | (banc << 1));
      rv = Eeprom::m_adresse_peripherique;
   }
   return rv;
}
...
/*                                                                            */
/******************************************************************************/
uint8_t Eeprom::lecture(const uint16_t adresse, uint8_t* donnee) {
   uint8_t rv = 0;
...
uint8_t
Eeprom::lecture(const uint16_t adresse, uint8_t* donnee, uint8_t longueur) {
   uint8_t twcr;
...
/*                                                                            */
/******************************************************************************/
uint8_t Eeprom::ecriture(const uint16_t adresse, const uint8_t donnee) {
   //______________ Attente de la fin d'un cycle d'ecriture ______________
   for(;;) {
      TWCR = _BV(TWINT) | _BV(TWSTA) | _BV(TWEN); // Condition de depart
      while((TWCR & _BV(TWINT)) == 0) // Attente de fin de transmission
...
uint8_t Eeprom::ecriture(const uint16_t adresse,
                         uint8_t* donnee,
...
uint8_t Eeprom::ecrire_page(const uint16_t adresse,
                            uint8_t* donnee,
                            const uint8_t longueur) {
   uint16_t addr_fin;
   uint8_t rv = 0;
